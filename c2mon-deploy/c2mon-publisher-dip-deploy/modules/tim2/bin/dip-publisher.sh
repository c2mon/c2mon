...
# Date:   July 2012
#
# This script is used to launch the DIP Publisher on a linux
# server. 
# Please be aware that this script will NOT run from 
# everywhere but from the APP_HOME directory.  


# Notification list, in case of problems reported by silentcheck
...
PID_FILE=$APP_HOME/tmp/$PROCESS_HOST/dippublisher_$PROCESS_NAME.pid
...
PROCESS_start() {
	...
	if [ $? -eq 1 ] ; then
	 ...
	 echo_warning
         echo "A DIP publisher $PROCESS_NAME process seems to be running on host $PROCESS_HOST. Stop it first."
...
really_start() {
  ...
  if [ $? -eq 1 ] ; then
    echo_failure
    echo "DIP Publisher $PROCESS_NAME process could not be started."
    echo
...
PROCESS_stop() {
  ...
  #if PID file exists
  if [ -f $PID_FILE ] ; then
	echo -n "Stopping the DIP publisher $PROCESS_NAME process on ${PROCESS_HOST} ..."
	pid=`cat $PID_FILE | awk {'print $1'}`
...
PROCESS_silentcheck() {
  if [ -f $PID_FILE ]; then
    ...
    runs $pid
    if [ $? -ne 0 ]; then
      cp -Rfp ${APP_HOME}/log/${PROCESS_NAME}.* ${APP_HOME}/${PROCESS_NAME}_`date +log_%Y-%m-%d_%H:%M:%S`
      rm ${APP_HOME}/log/${PROCESS_NAME}.*
      PROCESS_restart
      echo "The status check script has restarted the DIP publisher process $PROCESS_NAME on $HOSTNAME. Please check why the process was down (crash) and if it is starting up correctly. A backup of the log files was created in ${APP_HOME}/${PROCESS_NAME}_`date +log_%Y-%m-%d_%H:%M:%S`" | mail -s "Automatic DIP publisher $PROCESS_NAME process restart on $HOSTNAME" $NOTIFY
    fi
  fi
}
...
PROCESS_printBasicUsageInfo() {
    echo "*****************************************************************"
    echo " usage:"
    echo " $0 [start|stop|restart|status|silentcheck] <publisher-name>"
    echo " e.g: $0 start DIPPub01"
    echo
    echo " Known publishers are: DIPPub01, DIPPub02"
    echo "*****************************************************************"
}
...
if [ "$PROCESS_NAME" == "" -o "$PROCESS_NAME" != "DIPPub01" -a "$PROCESS_NAME" != "DIPPub02" ] ; then
  PROCESS_printBasicUsageInfo
else 
