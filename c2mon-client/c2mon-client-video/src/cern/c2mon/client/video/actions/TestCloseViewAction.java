...
public class TestCloseViewAction extends AbstractAction {
  
  /** Serial version UID */
  private static final long serialVersionUID = 179025730064116228L;

  /** Singelton instance */
  private static TestCloseViewAction instance = null;
   ...
   */
  protected TestCloseViewAction() {
    super("Test Closing Views");
   ...
   */
  public static TestCloseViewAction getInstance() {
    if (instance == null)
      instance = new TestCloseViewAction();
  ...
  public void actionPerformed(ActionEvent arg0) {
    ...
    new Thread(new Runnable() {
      ...
      public void run() {
        ...
        TimVideoViewer.getInstance().getVideoMainHandler().setActiveVideoConncetion(openViews[0]);
      
        for (int i = 1; i < openViews.length; i++) {
          System.out.println("Start " + openViews[i].getCamera());
          final int camera = openViews[i].getCamera();
          System.out.println("Start " + camera);
          TimVideoViewer.getInstance().getVideoMainHandler().addVideoRequest(openViews[i]);
        }
        
        count = 0;
        for (int i = 2; i < openViews.length; i++) {
          try {Thread.sleep(5000);} catch (InterruptedException e) {}
          TimVideoViewer.getInstance().getVideoMainHandler().closeVideoConnection(openViews[i]);
        }
      }
    }).start();
  }
