/******************************************************************************
 * Copyright (C) 2010-2020 CERN. All rights not expressly granted are reserved. This file is part of the CERN Control
...
public final class JmsProxyImpl implements JmsProxy, JmsSubscriptionHandler {
    ...
    /**
     * Time-to-live used for all messages sent to the server: notice this needs to be quite large to account for
    ...
    public JmsProxyImpl(final JmsConnectionHandler jmsConnectionHandler,
            final SlowConsumerListener slowConsumerListener, final EnqueuingEventListener enqueuingEventListener,
        ...
        heartbeatTopicWrapper = new HeartbeatTopicWrapper(slowConsumerListener, enqueuingEventListener,
                topicPollingExecutor, properties);
        supervisionTopicWrapper = new SupervisionTopicWrapper(slowConsumerListener, enqueuingEventListener,
                topicPollingExecutor, properties);
        broadcastTopicWrapper = new BroadcastTopicWrapper(slowConsumerListener, enqueuingEventListener,
                topicPollingExecutor, properties);
        alarmTopicWrapper = new AlarmTopicWrapper(slowConsumerListener, enqueuingEventListener, topicPollingExecutor,
                properties);
        tagTopicWrapper = new TagTopicWrapper(slowConsumerListener, enqueuingEventListener, topicPollingExecutor,
                properties);
    ...
    @Override
    public void replaceListener(final TagUpdateListener registeredListener,
            final TagUpdateListener replacementListener) {
    ...
    public <T extends ClientRequestResult> Collection<T> sendRequest(final JsonRequest<T> jsonRequest,
            final String queueName, final int timeout, final ClientRequestReportListener reportListener)
            throws JMSException {
                ...
                try {
                    ...
                    while (jmsConnectionHandler.isConnected() && !jmsConnectionHandler.isShutdownRequested()) {
                        ...
                        if (replyMessage == null) {
                            log.error("No reply received from server on ClientRequest. I was waiting for " + timeout
                                    + " milliseconds..");
                        ...
                        if (replyMessage instanceof ObjectMessage) {
                            ...
                            Collection<T> resultCollection = handleJsonResponse(textMessage, jsonRequest,
                                    reportListener);
    ...
    /**
     * In case a JsonResponse has been received. This can either be the final Result or a Report on the progress of the
     * request.
     * 
     * @param jsonMessage the received message.
     * @param jsonRequest the original request. useful to decode the message
     * @param reportListener informed in case a Report is received. Can be null in case no one cares about the progress
     *            of this request.
     * @param <T> type returned depends on the ClientRequest type.
     * @return a Collection of ClientRequestResults.
     * @throws JMSException if problem subscribing
     */
    private <T extends ClientRequestResult> Collection<T> handleJsonResponse(final TextMessage jsonMessage,
            final JsonRequest<T> jsonRequest, final ClientRequestReportListener reportListener)
     ...
     */
    private void handleJsonReportResponse(final ClientRequestReport report,
        ...
        if (report.isErrorReport()) {
            ...
            throw new RuntimeException(
                    "Error report received from server on client request: " + report.getErrorMessage());
    ...
    @Override
    public void registerUpdateListener(TagUpdateListener serverUpdateListener,
