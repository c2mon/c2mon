...
class JmsConnectionHandler {
    ...
    private final EnqueuingEventListener enqueuingEventListener;
    ...
    private final C2monClientProperties properties;
    ...
    public JmsConnectionHandler(@Qualifier("clientJmsConnectionFactory") final ConnectionFactory connectionFactory,
                                ...
                                @Qualifier("topicPollingExecutor") final ExecutorService topicPollingExecutor, final C2monClientProperties properties) {
                    ...
                    try {
                        if (refreshSubscriptions || connected) {
                            ...
                            if (topicToWrapper.containsKey(topicName)) {
                                ...
                                MessageListenerWrapper wrapper = new MessageListenerWrapper(topicRegistrationDetails.getId(), serverUpdateListener, properties.getHighListenerQueueSize(),
                                        slowConsumerListener, enqueuingEventListener, topicPollingExecutor);
                                wrapper.start();
