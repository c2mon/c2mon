...
public class Clock {
  ...
  /**
   * 
   * @return The time elapsed since the clock was (last) resumed. If it is currently
   *         paused it will return zero. This does NOT include the time BEFORE
   *         it was resumed
   */
  private synchronized long elapsedSinceResume() {
    if (this.paused) {
      ...
      // The current time minus the time of when it was last resumed equals the time which have elapsed (in real time)
      final long realtimeSinceUnpausing = (System.currentTimeMillis() - this.resumedAtRealtime);
      
      // Converts the time into fictional time, which is changed by the clock speed 
      return (long) (realtimeSinceUnpausing * this.speedMultiplier);
    }
  }
  
  /**
   * @return the total elapsed time in milliseconds (with consideration for the speed multiplier)
   */
  ...
  public synchronized void pause() {
    ...
    if (!this.paused) {
      // Keeps the count of how much fictional time have elapsed, and adds it to the total
  ...
  /**
   * Stops the clock.
   * Sets the time to the beginning.
   */
   ...
   * 
   * @param clock The parameter to synchronize with
   */
