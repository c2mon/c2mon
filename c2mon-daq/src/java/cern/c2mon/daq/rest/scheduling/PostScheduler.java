...
public class PostScheduler extends RestScheduler {
  ...
  public HttpStatus sendValueToServer(Long id, Object value) {
    ...
    if (this.contains(id)) {
      ...
      RestPostAddress hardwareAddress = getHardwareAddress(id);
      ...
      if(idToTask.get(id).cancel()){
        ...
        timer.purge();
        timer.schedule(newTask, hardwareAddress.getFrequency());
  ...
  public void addTask(Long id) {
    ...
    timer.schedule(task, hardwareAddress.getFrequency());
  ...
  private RestPostAddress getHardwareAddress(Long id) {
