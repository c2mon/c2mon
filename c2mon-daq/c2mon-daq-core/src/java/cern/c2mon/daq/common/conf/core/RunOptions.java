...
//      setUseEquipmentLoggers(true);
//      if (commandParamsHandler.hasParam("-eqAppendersOnly")) {
//        setUseEquipmentAppendersOnly(true);
//      }
//    }
//
    // check if the filtering should be turned off (default is on)
    setFilterMode(environment.getProperty("c2mon.daq.filter.enabled", Boolean.class));
    if (!filterMode) {
      LOGGER.info("The DAQ process is starting without filtering (no JMS connections will be opened with Filter module)");
    }

    setEqLoggers(environment.getProperty("c2mon.daq.logging.useEquipmentLoggers", Boolean.class));
    if (eqLoggers) {
      setEqAppendersOnly(environment.getProperty("c2mon.daq.logging.useEquipmentAppendersOnly", Boolean.class));
    }
  }
  ...
  /**
   * This method sets the setUseEquipmentAppendersOnly boolean flag
   *
