...
public abstract class ProcessMessageReceiver {
  ...
  /**
   * Send the current values of all data tags to the server (retrieved from DAQ
   * memory - no equipment specific call)
   *
   ...
   * @throws JMSException Throws a JMS exception if the sending of the answer
   *           fails.
   */
  public abstract void sendDataTagValueResponse(final SourceDataTagValueResponse sourceDataTagValueResponse, final Topic replyTopic, Session session)
      throws JMSException;
   ...
   * @throws JMSException Throws a JMSException if the sending of the answer
   *           message failed.
   */
  public abstract void sendCommandReport(final SourceCommandTagReport sourceCommandTagReport, final Destination destination, final Session session)
      throws JMSException;
   ...
   * @param destination The JMS destination.
   * @param session The JMS session.
   * @throws JMSException Throws a JMSException if the sending of the message
   *           failed.
   ...
   * @throws InstantiationException Called if no default constructor could be
   *           called from the newInstance method via reflection.
   ...
   */
  public abstract void sendConfigurationReport(final ConfigurationChangeEventReport configurationChangeEventReport, final Destination destination,
      final Session session) throws TransformerException, ParserConfigurationException, IllegalAccessException, InstantiationException, JMSException;

  /**
   * The method simply distinguish between different possible types of messages
   * coming to the DAQ.
   *
  ...
  public final void onReconfigureProcess(final Document doc, final Destination destination, final Session jmsSession) {
    ...
    try {
      ...
      try {
        ...
        /*
         * This should not happen if there is no major bug in the code which
         * sets the fields of the objects.
         */
        configurationReport.setError(StackTraceHelper.getStackTrace(e));
      } catch (NoSuchMethodException e) {
        configurationReport.setError("The NoSuchMethodException might be caused through a not " + "supported change in the XML. The reflective call of"
            + "the right method will fail then.\n" + StackTraceHelper.getStackTrace(e));
   ...
   */
  public void onSourceDatatagValueUpdateRequest(final Document doc, final Topic replyTo, final Session session) throws JMSException {
    requestLock.writeLock().lock();
    try {
      ...
      if (replyTo == null) {
        ...
        if (rootEl != null) {
          ...
          SourceDataTagValueResponse sourceDataTagValueResponse = requestController.onSourceDataTagValueUpdateRequest(sdtValueRequest);
  ...
  public void onExecuteCommand(final Document doc, final Destination destination, final Session session) {
    ...
    try {
      ...
      if (destination == null) {
        LOGGER.warn("\tthe \"replyTo\" topic of the JMS command execution request has not been specified. The execution will be skipped..");
      } else {
        Element rootEl = doc.getDocumentElement();
        if (rootEl != null) {
