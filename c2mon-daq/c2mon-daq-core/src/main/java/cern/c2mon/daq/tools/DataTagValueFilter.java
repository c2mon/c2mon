...
public class DataTagValueFilter {
   ...
   * SourceDataTag should be sent to the application server or not. The decision
   * is taken based on the deadband specification of the considered tag and
   * assumes that the new update is valid (= quality OK).
   *
   * @param currentTag the current of the tag
   * @param newTagValue new value of the SourceDataTag, received from a data
   *          source.
   ...
   * This method is responsible for checking if the new value of the particular
   * SourceDataTag should be sent to the application server or not. The decision
   * is taken based on the deadband specification of the considered tag
   *
   * @return True if the value is filtered else false.
   */
  private boolean isValueDeadbandFiltered(final SourceDataTag currentTag, final ValueUpdate update, final SourceDataTagQuality newSDQuality) {
    ...
    if (currentTag.getAddress().isProcessValueDeadbandEnabled()) {
      ...
      if (isCurrentValueAvailable(currentTag) && (currentTag.getCurrentValue().getQuality().getQualityCode() == newSDQuality.getQualityCode())) {
        ...
        if (TypeConverter.isNumber(currentTag.getDataType())) {
          if (isCurrentValueAvailable(currentTag)) {
            ...
            case DataTagDeadband.DEADBAND_PROCESS_ABSOLUTE_VALUE_DESCR_CHANGE:
              ...
              // check if the value description has changed, if yes - then do
   ...
   * Avoid sending twice (one by one) 2 invalid tags with the same quality code
   * and description
   *
   * Currently used by EquipmentSenderInvalid
   *
   * @return <code>FilterType</code>, if this the new quality is a candidate for
   *         being filtered out it will return the reason if not it will return
  ...
  public FilterType isCandidateForFiltering(final SourceDataTag currentTag, final ValueUpdate castedUpdate, final SourceDataTagQuality newSDQuality) {
    ...
    if (currentSDValue != null) {
      ...
      if (isOlderUpdate(newSDQuality, currentSDValue.getQuality(), castedUpdate.getSourceTimestamp(), currentSDValue.getTimestamp().getTime())) {
        ...
        if ((result == FilterType.REPEATED_INVALID) || (result == FilterType.REPEATED_VALUE)) {
          // The value will be filtered out by result (REPEATED_INVALID,
          // REPEATED_VALUE)
          return result;
        }
        else {
   ...
   *
   * @param currentTag The current tag object of the {@link SourceDataTag} that
   *          shall be updated
   * @param newValue The new update value that we want set to the tag
   * @param newTagValueDesc The new update value description
   * @param newSDQuality The new quality info for the {@link SourceDataTag} that
   *          shall be updated
   ...
   *         <code>FilterType.NO_FILTERING</code>
   */
  private FilterType isRepeatedValue(final SourceDataTag currentTag, final ValueUpdate update, final SourceDataTagQuality newSDQuality) {
    ...
    // The two values are both null or equal. Now we check for redundant Value
    // Description information
    if ((filtering = isDifferentValueDescription(currentSDValue, update.getValueDescription())) != null) {
      return filtering;
    }

    // Current and new Values and Value Descriptions are both null or equal! Now
  ...
  /**
   * Helper method which compares the values of the new DataTag and the vale of
   * the previous one. The type of the value is complex which means it is either
   * an array or an arbitrary object.
   *
   * @param currentTag
   * @param newValue
  ...
  private FilterType isDifferentValueDescription(SourceDataTagValue currentSDValue, final String newTagValueDesc) {
    ...
    if (!currentSDValue.getValueDescription().equalsIgnoreCase(newTagValueDesc)
      ...
      /*
       * Note 1: currentSDValue.getValueDescription() will never be null Note 2:
       * if getValueDescription is empty and newTagValueDesc is null we get not
       * equal but for us will be equal (no value) so we take care of this
       * special case and continue the checks
       */
  ...
  private FilterType isDifferentDataTagQuality(SourceDataTagValue currentSDValue, final SourceDataTagQuality newSDQuality) {
    FilterType filtering = null;
      ...
      if ((currentSDValue.getQuality().getQualityCode() == newSDQuality.getQualityCode())) {
        ...
        if (!newSDQuality.isValid()) {
          ...
          if (currentSDValue.getQuality().getDescription() == null) {
            // If description is null we cannot compare so we check directly if
  ...
  /**
   * Returns true if the difference of the provided numbers is smaller than the
   * value deadband.
   *
   ...
   * Quality - New TS < Current TS + Current has DATA_UNAVAILABLE Quality + New
   * Bad Quality
   *
  ...
  protected boolean isOlderUpdate(final SourceDataTagQuality newSDQuality, final SourceDataTagQuality currentSDQuality, final long newTimestamp,
    ...
    if (newTimestamp < currentTimestamp) {
      ...
      if (currentSDQuality.getQualityCode() == SourceDataTagQualityCode.DATA_UNAVAILABLE) {
        ...
        // If current tag was unavailable we allow sending tag value with good
