...
public class DataTagValueFilter {
	...
	/**
	 * We try to calculate the delta on a per-data type basis so as to reduce the risk of precision loss when converting to double.
	 * @param currentValue The current value.
	 * @param newValue The new value.
	 * @return <code>null</code> if no sensible delta could be calculated (e.g. loss of precision conversion), a Double with the delta otherwise.
	 */
	private Double calculateDelta(final Number currentValue, final Number newValue) {
		...
		if (currentValue instanceof Integer) {
			if( willCausePrecisionLoss(currentValue.intValue()) || willCausePrecisionLoss(newValue.intValue())){
				log.trace("Possible loss of precision detected on incoming integer values when evaluating against a float deadband");
				return null;
			...
			if(willCausePrecisionLoss(currentValue.longValue()) || willCausePrecisionLoss(newValue.longValue())){
				log.trace("Possible loss of precision detected on incoming long values");
				return null;
			}
			return Math.abs(new Double(n));
	...
	/**
	 * Check if the given integer could cause a loss of precision when converted to float.
	 * @param val The given integer.
	 * @return <code>true</code> if the given integer could cause a loss of precision when converted to float.
	 */
	...
	/**
	 * Check if the given long could cause a loss of precision when converted to double.
	 * @param val The given long.
	 * @return <code>true</code> if the given long could cause a loss of precision when converted to double.
	 */
