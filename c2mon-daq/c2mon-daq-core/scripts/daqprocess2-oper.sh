...
# The OPC daq host
DAQ_OPC_HOST=cs-ccr-tim1

# set daqs primary host
DAQ_PRIMARY_HOST=cs-ccr-tim6

# set daqs secondary host
DAQ_SECONDARY_HOST=cs-ccr-tim9

# Used to check whether the DAQ that is going to be stopped is
# maybe still running on the obsolete DAQ host
OBSOLETE_HOST=cs-ccr-tim4
...
# Build up the path used to store the PID process file
...
really_start() {
  export DIM_DNS_NODE=dipns1.cern.ch
  ...
  ~/dist/daqprocess2/build/bin/daqprocess2 >${DAQ_LOG_HOME}/${PROCESS_NAME}.out.log 2>&1 &
    ...
    if [ $USE_XML_PROTOCOL -eq 0 ] ; then
     ...
     if [ $USE_XML_PROTOCOL -eq 0 ] ; then
       ...
       if [ $? -eq 0 ] ; then
         ...
         TIMDAQ_EchoXMLFeedback -1 "Process ${PROCESS_NAME} does not seem to be running"
       fi
...
# Restart: stops the DAQ, starts it again
TIMDAQ_restart() {
  TIMDAQ_stop
  sleep 1
  TIMDAQ_start
...
#
# Allows exectuting one of the commands supported by the DAQ script
# (start/stop/status) from a remote host.
#
