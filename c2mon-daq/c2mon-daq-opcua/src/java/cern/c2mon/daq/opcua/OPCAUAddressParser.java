 ...
 * 
 */
public class OPCAUAddressParser {
     ...
     *         address String.
     */
    public List<OPCUAAddress> parseAddressString(final String address) {
        List<OPCUAAddress> addresses = new ArrayList<OPCUAAddress>(2);
        try {
            ...
            OPCUAAddress primaryAddress = createOPCAddress(
                    uris[0], usersAtDomains[0], passwords[0], 
                    serverTimeout, serverRetryTimeout, aliveWriter);
            addresses.add(primaryAddress);
            if (uris.length > 1) {
                OPCUAAddress alternativeAddress = createOPCAddress(
     ...
     */
    private OPCUAAddress createOPCAddress(final String uri, final String userAtDomain, final String password, final int serverTimeout, final int serverRetryTimeout, final boolean aliveWriter) {
        OPCUAAddress primaryAddress;
        try {
            primaryAddress = new OPCUAAddress.Builder(
