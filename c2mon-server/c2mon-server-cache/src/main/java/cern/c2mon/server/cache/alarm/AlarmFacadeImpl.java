...
/**
  ...
  private Alarm update(final Alarm alarm, final Tag tag) {
    ...
    if (alarmCacheObject.getTimestamp().equals(new Timestamp(0))
      ...
      log.trace(new StringBuffer("update(): alarm " + alarmCacheObject.getId() + " changed STATE to ") + newState);
    ...
    if (!alarmCacheObject.getInfo().equals(additionalInfo)) {
      log.trace("update(): alarm " + alarmCacheObject.getId() + " changed from INFO to " + additionalInfo);
      alarmCacheObject.setInfo(additionalInfo);
    ...
    log.trace("update(): alarm " + alarmCacheObject.getId() + " has not changed.");
