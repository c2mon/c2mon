...
public class TransportConnector implements Connector {
	...
	/**
	 * Method that query the cluster to list all the present indices.
	...
	public void indexTags(List<TagES> tags) {
		...
		if (tags == null) {
			...
			for (TagES tag : tags) {
				indexTag(tag);
				aliases.put(generateAliasName(tag.getTagId()), tag); // 1 by 1 so to be launched after.
			}
			...
			client.admin().indices().prepareRefresh().execute().actionGet(); // for quasi real time retrieval.
			client.admin().cluster().prepareHealth().setWaitForYellowStatus().execute().actionGet();
		}
	}
