...
public class TransportConnectorTest {
	...
	public void tidyUp() {
		log.info("@After");
		clean(connector.getClient(), connector.getIndices());
		connector.setAliases(new HashSet<String>());
	...
	public void testBulkAddAlias() {
		...
		connector.handleIndexQuery(createIndexQuery(connector.getClient()), "c2mon_2015-12", connector.getMonthIndexSettings(), connector.generateType(tag.getDataType()), tag.getMapping());
