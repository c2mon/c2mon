...
CREATE TABLE ALARM (
  ...
  ALARMSOURCETIME  TIMESTAMP DEFAULT '1970-01-02 00:00:01',
  ALARMINFO        VARCHAR(100),
  ALARMCONDITION   VARCHAR(500),
  ALARMMETADATA    VARCHAR(4000),
  ALARMOSCILLATION INTEGER
);
...
CREATE VIEW COMMFAULTTAG (COMMFAULTID, EQID, EQNAME, EQTYPE, EQSTATETAG, EQALIVETAG) AS
SELECT
    a.EQCOMMFAULT_TAGID COMMFAULTID,
    a.EQID EQID,
    a.EQNAME EQNAME,
   'SUBEQ' EQTYPE,
    a.EQSTATE_TAGID EQSTATETAG,
    a.EQALIVE_TAGID EQALIVETAG
FROM EQUIPMENT a, EQUIPMENT b
WHERE
    a.EQ_PARENT_ID IS NOT NULL AND
    a.eq_parent_id = b.EQID
UNION
SELECT
    EQCOMMFAULT_TAGID COMMFAULTID,
    EQID EQID,
    EQNAME EQNAME,
    'EQ' EQTYPE,
    EQSTATE_TAGID EQSTATETAG,
    EQALIVE_TAGID EQALIVETAG
FROM EQUIPMENT
WHERE
    EQ_PARENT_ID IS NULL;


CREATE VIEW STATETAG (STATETAGID, SUP_ID, SUP_TYPE, ALIVETAGID, COMMFAULTID) AS
  SELECT
...
UNION
  SELECT
    EQSTATE_TAGID STATETAGID,
    EQID SUP_ID,
    'EQ' SUP_TYPE,
    EQALIVE_TAGID ALIVETAGID,
    EQCOMMFAULT_TAGID COMMFAULTID
  FROM PROCESS p, EQUIPMENT e
  WHERE
    EQSTATE_TAGID IS NOT NULL AND
    PROCID = EQ_PROCID
UNION
  SELECT
    child.EQSTATE_TAGID STATETAGID,
    child.EQID SUP_ID,
    'SUBEQ' SUP_TYPE,
    child.EQALIVE_TAGID ALIVETAGID,
    child.EQCOMMFAULT_TAGID COMMFAULTID
  FROM EQUIPMENT child, EQUIPMENT parent
  WHERE
   child.EQSTATE_TAGID IS NOT NULL AND
   child.EQ_PARENT_ID = parent.EQID;
...
ALTER TABLE DEVICEPROPERTY ADD
CONSTRAINT FK_DVP_PROPID_PROPERTY FOREIGN KEY (DVPPROPID) REFERENCES PROPERTY (PROPID) ON DELETE CASCADE;
...
ALTER TABLE DEVICECOMMAND ADD
CONSTRAINT FK_DVC_CMDID_COMMAND FOREIGN KEY (DVCCMDID) REFERENCES COMMAND (CMDID) ON DELETE CASCADE;
...
ALTER TABLE PROPERTYFIELD ADD
CONSTRAINT FK_PRF_PRFFIELDID_FIELD FOREIGN KEY (PRFFIELDID) REFERENCES FIELD (FIELDID) ON DELETE CASCADE;
