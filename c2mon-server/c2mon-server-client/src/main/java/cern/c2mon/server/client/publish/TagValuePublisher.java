...
/**
 * This class implements the <code>AlarmAggregatorListener</code> interface for sending tag value updates to the tag JMS
 * destination topics. The update information is transmitted as GSON message with the <code>TransferTagValue</code>
 * class. This class implements the <code>ConfigurationUpdateListener</code> interface for sending configuration updates
 * to the tag JMS destination topics. The update information is transmitted as GSON message with the
 * <code>TransferTag</code> class.
 *
...
public class TagValuePublisher
        implements AlarmAggregatorListener, ConfigurationUpdateListener, Publisher<TagWithAlarms> {
    ...
    public TagValuePublisher(@Qualifier("clientTopicPublisher") final JmsSender jmsSender,
            final AlarmAggregatorRegistration alarmAggregatorRegistration, final AliveTimerFacade aliveTimerFacade,
            final ConfigurationUpdate configurationUpdate, final TagFacadeGateway pTagFacadeGateway,
            final TagLocationService tagLocationService, final ClientProperties properties) {
    ...
    /**
     * Generates for every notification a <code>TransferTagValue</code> object which is then sent as serialized GSON
     * message trough the dedicated JMS client tag topic.
     * 
     * @param tag the updated Tag
     * @param alarms the new values of the associated alarms; this list is null if no alarms are associated to the tag
     */
    @Override
    public void notifyOnUpdate(final Tag tag, final List<Alarm> alarms) {
        ...
        try {
            ...
            log.error("notifyOnUpdate - Error publishing tag update to topic for tag " + tagWithAlarms.getTag().getId()
    ...
    public void publish(final TagWithAlarms tagWithAlarms) {
        ...
        TransferTagImpl tagValue = TransferObjectFactory.createTransferTag(tagWithAlarms,
                aliveTimerFacade.isRegisteredAliveTimer(tagWithAlarms.getTag().getId()), topic);
        log.trace("publish - Publishing tag update to client: " + TransferTagSerializer.toJson(tagValue));

        jmsSender.sendToTopic(TransferTagSerializer.toJson(tagValue),
        ...
        try {
            ...
            try {
                ...
                TransferTagImpl tag = TransferObjectFactory.createTransferTag(tagWithAlarms,
                        aliveTimerFacade.isRegisteredAliveTimer(tagId), topic);

                log.trace("notifyOnConfigurationUpdate - Publishing configuration update to client: "
                        + TransferTagSerializer.toJson(tag));

                jmsSender.sendToTopic(TransferTagSerializer.toJson(tag), topic);
            } catch (JmsException e) {
                log.error("notifyOnConfigurationUpdate - Error publishing configuration update to topic for tag "
                        + tagWithAlarms.getTag().getId() + " - submitting for republication", e);
                republisher.publicationFailed(tagWithAlarms);
            }
        } finally {
