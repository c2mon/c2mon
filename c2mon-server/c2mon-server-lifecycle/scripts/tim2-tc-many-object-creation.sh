...
COMMAND_OPTIONS="-Xms1024m -Xmx1024m -XX:+PrintGCDetails -XX:InitialSurvivorRatio=4 -XX:MinSurvivorRatio=4 -XX:-UseAdaptiveSizePolicy -XX:+UseParallelGC -XX:NewRatio=1 -Dcom.tc.l1.cachemanager.percentageToEvict=10 -Dcom.tc.l1.cachemanager.threshold=70 -Dcom.tc.l1.cachemanager.monitorOldGenOnly=false -Dserver.process.name=$PROCESS_NAME -Dtc.config=$TERRACOTTA_CONFIG -Dlog4j.configuration=$LOG4J_CONF_FILE -Dtim.log.dir=$LOG_DIR -Dtim.properties.location=$TIM_PROPERTIES -classpath ${REQUIRED_LIBS}"
#-Dcom.tc.l1.cachemanager.criticalObjectThreshold=100000 -XX:+UseConcMarkSweepGC

echo run with options: $COMMAND_OPTIONS

dso-java.sh $COMMAND_OPTIONS cern.tim.server.startup.ServerStartup
