 ...
 */
@Slf4j
public class MultiThreadedCacheListener<T extends Cacheable> implements CacheListener<T>, Lifecycle {
   ...
   */
  private CacheListener<T> cacheListener;
   ...
   * Constructor.
   *
   * @param cacheListener  the listener wrapped by this class
   *                       (the module listener)
    ...
    private void callCorrectMethod(final ObjectAndMethod objectAndMethod) {
      try {
        ...
        if (method.equals(SupportedMethods.ON_UPDATE)) {
          cacheListener.notifyElementUpdated(objectAndMethod.cacheable);
        }
        else {
          cacheListener.confirmStatus(objectAndMethod.cacheable);
        }
      }
      catch (Exception e) {
